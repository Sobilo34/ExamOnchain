...
export default function LecturerCoursesPage() {
  ...
  const [courses, setCourses] = useState<
    { id: number; title: string; code: string; level: string; term: string; joinCode: string | null }[]
  >([]);
  ...
  return (
    <AppShell role="lecturer" email={me.email} wallet={me.smartAccountAddress}>
      <div className="flex items-center justify-between">
        ...
        <Link href="/lecturer/courses/new" className="rounded-lg bg-slate-900 px-4 py-2 text-sm text-white">
          New course
        </Link>
      </div>
      <ul className="mt-6 space-y-3">
        {courses.map((c) => (
          <li key={c.id} className="rounded-lg border border-slate-200 bg-white p-4">
            <Link href={`/lecturer/courses/${c.id}/edit`} className="font-medium text-slate-900 hover:underline">
              {c.code} — {c.title}
            </Link>
            ...
            <div className="mt-2 flex gap-3 text-sm">
              <Link className="text-sky-700 underline" href={`/lecturer/courses/${c.id}/roster`}>
                Roster
              </Link>
              <Link className="text-sky-700 underline" href={`/lecturer/courses/${c.id}/materials`}>
                Materials
              </Link>
              <Link className="text-sky-700 underline" href={`/lecturer/courses/${c.id}/exams/new`}>
                New exam
              </Link>
            </div>
          </li>
